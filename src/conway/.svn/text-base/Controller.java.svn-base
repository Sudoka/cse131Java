...
public class Controller extends Tool {
	...
	public void mousePressed(MouseEvent e) {
		// Convert click to spot on board
		int width = view.getPanel().getWidth();
		int height  = view.getPanel().getHeight();
		int x=e.getX()*size/width;
		int y =e.getY()*size/height;
		if (conway.isAlive(x,y)) {
			updateViewAndModel(false,x,y);
		}
		else {
			updateViewAndModel(true,x,y);
		}
